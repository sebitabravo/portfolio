...
function sameTokens(actual: string[], expected: string[]): boolean {
  return (
    ...
    [...actual].sort().every((token, index) => token === [...expected].sort()[index])
  )
}
...
export function validateVercelConfig(input: unknown): {
  ...
  const rules = isRecord(config) && Array.isArray(config.headers) ? (config.headers as HeaderRule[]) : []
  const globalRules = rules.filter((rule) => isRecord(rule) && rule.source === "/(.*)")

  if (globalRules.length !== 1) {
    return {
      errors: [`expected exactly one global /(.*) rule, found ${globalRules.length}`],
    }
  }

  const rawHeaders = globalRules[0].headers
  if (!Array.isArray(rawHeaders)) return { errors: ["global /(.*) rule must contain a headers array"] }
  ...
  for (const [key, value] of requiredHeaders) {
    if (headers.get(key) !== value) errors.push(`required header ${key} must equal ${value}`)
  }

  const csp = headers.get("content-security-policy")
  if (!csp) return { errors: [...errors, "missing content-security-policy header"] }
  ...
  for (const [directive, expected] of fixedDirectives) {
    ...
    if (!actual || !sameTokens(actual, expected)) {
      errors.push(`CSP directive ${directive} must contain exactly: ${expected.join(" ") || "no values"}`)
    }
  }
  ...
  if (scriptSources) {
    ...
    for (const source of scriptSources) {
      ...
      if (hash) scriptHashes.add(hash[1])
        ...
        errors.push(`CSP directive script-src contains unexpected source: ${source}`)
    }
    if (
      !sameTokens(
      ...
      errors.push("CSP directive script-src must contain only the approved non-hash sources")
    }
  } else {
...
function isExecutableScriptType(type: string | undefined): boolean {
  ...
  return normalized === "" || normalized === "module" || javascriptMimeTypes.has(normalized.split(";", 1)[0].trim())
}
...
export function extractExecutableScriptBodies(html: string): string[] {
  ...
  while (opening.exec(html)) {
    ...
    if (!hasSrc && isExecutableScriptType(type)) bodies.push(html.slice(end + 1, close.index))
    opening.lastIndex = closing.lastIndex
  }
  return bodies
}

export function compareCspHashParity(configured: Set<string>, htmlDocuments: string[]) {
  const generated = new Set(htmlDocuments.flatMap(extractExecutableScriptBodies).map(hashScript))
  return {
    ...
    executableScripts: htmlDocuments.flatMap(extractExecutableScriptBodies).length,
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(`CSP verification could not read configuration or dist/: ${(error as Error).message}`)
    process.exitCode = 1
    return
  }
  ...
  if (parity.missing.length || parity.unused.length) {
    ...
    for (const hash of parity.missing) console.error(`  missing authorization: 'sha256-${hash}'`)
    for (const hash of parity.unused) console.error(`  unused authorization: 'sha256-${hash}'`)
    process.exitCode = 1
    return
  }
  console.log(

...
test.describe("Home Page", () => {
  test("core sections and project filters work", {
  ...
  test("mobile navigation, theme, language and reading progress work", {
    ...
    const mobileTrigger = page.locator(
      "[data-dropdown-trigger='mobile-nav-es']",
    );
    await mobileTrigger.click();
    await expect(
      mobileMenu.getByRole("link", { name: "Contacto", exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.locator("#contacto").getByText("Descargar CV", { exact: true }),
    ).toBeVisible();
  ...
  test("switching from English restores Spanish navigation anchors", {
    ...
    await expect(navigation.locator("[data-section='blog']")).toHaveAttribute(
      "href",
      "/blog",
    );
    await expect(
      navigation.locator("[data-section='proyectos']"),
    ).toHaveAttribute("href", "/#proyectos");
    await expect(
      navigation.locator("[data-section='experiencia']"),
    ).toHaveAttribute("href", "/#experiencia");
    await expect(
      navigation.locator("[data-section='educacion']"),
    ).toHaveAttribute("href", "/#educacion");
  });

  test("light theme keeps hero and navigation coherent", {
  ...
  test("reduced motion keeps content visible and stops loops", {
    ...
    const state = await page.evaluate(() => {
      ...
      const hiddenElements = Array.from(
        document.querySelectorAll("[data-animate], .hero-reveal"),
      ).filter(
        (element) =>
  ...
  test("progressive motion layer falls back safely and follows project scroll", {
    ...
    const motionStatus = await page
      .locator("[data-portfolio-motion]")
      .getAttribute("data-motion-status");
    await expect(page.locator("[data-hero-webgl]")).toHaveAttribute(
      "data-webgl-status",
      /^(ready|fallback)$/,
      { timeout: 10000 },
    ...
    await expect
      .poll(
        () =>
          page
            .locator("[data-project-scroll-meter] span")
            .evaluate((element) => {
              const match = element
                .getAttribute("style")
                ?.match(/scaleX\(([^)]+)\)/);
  ...
  test("availability stays a single hero status instead of covering the visual", {
  ...
  test("section bands keep one stack signal and add About conversion", {
    ...
    await expect(page.locator(".about-focus a")).toHaveAttribute(
      "href",
      "#contacto",
    );
    await expect(page.locator("#sobre-mi")).not.toContainText(
      "Stack Tecnológico",
    );

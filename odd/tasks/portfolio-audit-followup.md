...
## Tasks
- [x] Add a localized accessible region name/role to CertificationCarousel and regression assertions; keep existing keyboard/focus behavior. **Evidence:** RED 2/2 failed as expected; GREEN 2/2 passed after safe build; independent build/limits/bundle/CSP verification passed; full Chromium carousel spec 5/5 passed. **Commit:** pending.
- [ ] Make hero motion/WebGL lifecycle respond to live `prefers-reduced-motion` changes in both directions, including initial reduced-motion and teardown; preserve animation/resource cleanup.
- [ ] Centralize static hreflang alternate construction and reuse the exported `Locale` type in BlogCard and other local duplicate unions; preserve alternate URL ordering, x-default, and current route behavior.
